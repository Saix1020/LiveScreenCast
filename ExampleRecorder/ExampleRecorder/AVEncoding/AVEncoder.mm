...
-(void)addTimer:(NSNumber*)timer
{
    if(_times.count < 30){
        [_times addObject:timer];
    }
}

-(void)setLengthSize:(NSInteger)lengSize
{
    _lengthSize = (int)lengSize;
}
...
+ (AVEncoder*) encoderForHeight:(int) height andWidth:(int) width andFilePath:(NSString*)filePath
{
    AVEncoder* enc = [AVEncoder alloc];
    [enc initForHeight:height andWidth:width andFilePath:filePath];
    return enc;
...
- (void) initForHeight:(int)height andWidth:(int)width
{
    ...
    _times = [NSMutableArray arrayWithCapacity:30];
...
- (void) initForHeight:(int)height andWidth:(int)width andFilePath:(NSString*)filePath
{
    _height = height;
    _width = width;
    NSString* path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"params.mp4"];
    ...
    // swap between 3 filenames
    _currentFile = 1;
    _writer = [VideoEncoder encoderForPath:filePath Height:height andWidth:width];
}

-(void)setWriter:(VideoEncoder*)writer
{
    _writer = writer;
}
...
-(void)onParamsCompletionWithSampleFile:(NSString*)sampleFilePath
{
    if ([self parseParams:sampleFilePath])
    {
        if (_paramsBlock)
        {
            _paramsBlock(_avcC);
...
- (void) onParamsCompletionX
{
    // the initial one-frame-only file has been completed
    // Extract the avcC structure and then start monitoring the
    // main file to extract video from the mdat chunk.
    NSString* filepath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"sample.mp4"];

    if ([self parseParams:filepath])
//        if(1)
    {
        if (_paramsBlock)
        {
